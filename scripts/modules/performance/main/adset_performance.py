# modules/performance/main/adset_performance.py
...
ADSET_INSIGHTS_FIELDS = [
    "date_start", "adset_id", "campaign_id", "spend", "impressions", "reach", "clicks",
    "unique_clicks", "unique_inline_link_clicks", "actions", "action_values"
]

def get_adset_performance_df(ad_accounts: List[Dict[str, Any]], run_config: Dict[str, Any]) -> pd.DataFrame:
    """
    Extracts daily performance metrics at the ad set level using Meta's Marketing API.
    ...
    Returns:
        pd.DataFrame: Combined ad set performance data.
    """
    logger.info(f"📊 Fetching ad set performance for {len(ad_accounts)} ad accounts...")
    ...
    for acc in ad_accounts:
        ...
        try:
            ...
            logger.warning(f"⚠️ Failed to fetch ad set insights for account {account_id}: {e}", exc_info=True)

    df = pd.DataFrame(all_data)
    logger.info(f"📈 Total ad set performance rows loaded: {len(df)}")
    return df